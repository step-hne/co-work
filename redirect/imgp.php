 ...
 * Allows Facebook's OG scraper to read images that would otherwise be blocked
 * by CORS, geo-restrictions, or bot filters on the origin CDN.
 *
 * Usage: /imgp?u=<base64url-encoded-image-url>
 * Resizes to max 1200×630 and re-encodes as JPEG 85q if GD is available.
 */
...
// (DNS-resolution based check is skipped: unreliable on shared hosting)
if (filter_var($host, FILTER_VALIDATE_IP) !== false && imgp_is_private_ip($host)) {
    http_response_code(403);
    exit;
}
...
// ── GD compress / resize (max 1200×630 for OG) ───────────────────────────────
