...
function ensureShortlinksTable(mysqli $db): bool
{
    $stmt = mysqli_prepare(
        $db,
...
function createBuiltinShortUrl(string $longUrl, mysqli $db, string $ogTitle = '', string $ogImage = ''): ?string
{
    ...
    for ($attempt = 0; $attempt < 5; $attempt++) {
        ...
        $now  = time();
        $stmt = mysqli_prepare($db, 'INSERT IGNORE INTO shortlinks (code, long_url, og_title, og_image, created_at) VALUES (?, ?, ?, ?, ?)');
        if (!$stmt) {
            continue;
        }
        mysqli_stmt_bind_param($stmt, 'ssssi', $code, $longUrl, $ogTitle, $ogImage, $now);
        mysqli_stmt_execute($stmt);
...
try {
    ...
    $_short = createBuiltinShortUrl($urlEx, $link, $t, $fbimg);
    if ($_short === null) {
